...
export function equipEffectExplain(stat: string): Explain | null {
  ...
  const rows: Record<string, string[]> = {
    ...
    shieldBoostPct: [`护盾获得量提升词条值%；护盾总量上限为最大生命的 ${Number(EQUIP_EFFECTS.shield?.capPctOfMaxHp ?? 30)}%，超出部分不再叠加。`],
    magicShieldProcPct: [`释放技能时按词条概率消耗最大魔力 ${proc.magicShield?.mpCostPct}%，转化为等量护盾（受护盾上限约束）。`],
    lifestealShieldPct: ['吸血回复溢出生命上限的部分，按词条值% 转化为护盾。'],
