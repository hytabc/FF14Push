    ...
    <template v-else>
      ...
      <template v-if="state">
        <section class="panel"><div class="row"><h2>{{ state.status === 'cleared' ? '挑战成功' : state.status === 'failed' ? '挑战失败' : `阶段 ${state.phase + 1}` }}</h2><span>{{ (state.elapsedMs / 1000).toFixed(1) }} 秒 / {{ room.dungeon.enrageSeconds }} 秒</span></div><p v-if="state.reason" class="error">{{ state.reason }}</p>
          ...
          <div v-for="b in state.bosses" :key="b.id" class="boss"><div class="row"><strong>{{ b.name }}</strong><small>{{ Math.ceil(b.hp).toLocaleString() }} / {{ Math.ceil(b.maxHp).toLocaleString() }}</small><button :disabled="busy || !selected.length || state.status !== 'running'" @click="target(b.id)">选中英雄集火</button></div><HealthBar :value="b.hp" :max="b.maxHp" height="h-2.5" fill-class="bg-rose-500" /></div>
