...
interface Report { fighters: Snapshot[]; winner: number | null; result: string; durationMs: number; events: { at: number; actor: number; skill: string; damage: number; hp: number[] }[] }
...
async function challenge(id: number) { await act(async () => { report.value = (await http.post<{ report: Report }>('/pvp/challenge', { heroId: heroId.value, registrationId: id, key: crypto.randomUUID() })).data.report; cursor.value = 0; playing.value = true; await load() }) }
...
<template>
  <main class="dlc"><header><p class="eyebrow">八英雄远征 · 异步竞技场</p><h1>留下一位对手，迎接下一次挑战</h1><p>登记英雄与装备快照供其他玩家挑战。全自动1对1，180秒平局，无复活，无金币装备奖励。</p></header>
    ...
    <section class="panel"><h2>我的挑战英雄</h2><select v-model="heroId" aria-label="竞技场英雄"><option v-for="h in roster?.heroes" :key="h.id" :value="h.id">{{ h.name }} Lv.{{ h.level }} · {{ jobName(h.jobId) }}</option></select><button :disabled="busy || !heroId" @click="act(async () => { await http.post('/registrations', { heroId, kind: 'pvp' }); await load() })">登记 / 更新防守快照</button><div v-for="r in registrations.filter(r => r.userId === uid)" :key="r.id" class="row"><p>{{ r.snapshot.name }} · Lv.{{ r.snapshot.level }} · 已登记防守</p><button :disabled="busy" @click="act(async () => { await http.delete(`/registrations/${r.id}`); await load() })">撤回</button></div></section>
    <section v-if="report" class="panel"><h2>战报回放 · {{ report.result === 'draw' ? '平局' : report.fighters[report.winner!]?.name + ' 获胜' }}</h2><p>战斗时长 {{ (report.durationMs / 1000).toFixed(1) }}秒</p><div class="cards"><div v-for="(f,i) in report.fighters" :key="i"><h3>{{ f.name }} · {{ roles[f.role] }}</h3><progress :value="frame?.hp[i] ?? f.stats.max_hp" :max="f.stats.max_hp"/><small>{{ frame?.hp[i] ?? Math.round(f.stats.max_hp) }} HP</small></div></div><input v-model.number="cursor" type="range" min="0" :max="Math.max(0, report.events.length - 1)" aria-label="回放进度" style="width:100%"><button @click="playing = !playing">{{ playing ? '暂停' : '播放' }}</button><p v-if="frame">{{ (frame.at / 1000).toFixed(1) }}秒 · {{ report.fighters[frame.actor]?.name }} 使用 {{ frame.skill }}，造成 {{ frame.damage }}伤害</p></section>
    <h2>已登记对手</h2><p v-if="!registrations.some(r => r.userId !== uid)">暂无其他玩家登记，邀请伙伴登记防守英雄后即可挑战。</p><div class="cards"><article v-for="r in registrations.filter(r => r.userId !== uid)" :key="r.id" class="panel"><span class="badge">玩家 #{{ r.userId }}</span><h2>{{ r.snapshot.name }}</h2><p>Lv.{{ r.snapshot.level }} · {{ roles[r.snapshot.role] }} · {{ r.snapshot.jobId }}</p><p>生命 {{ Math.round(r.snapshot.stats.max_hp).toLocaleString() }} · 配装 {{ r.snapshot.items.length }}件</p><button class="primary" :disabled="busy || !heroId" @click="challenge(r.id)">挑战快照</button></article></div>
