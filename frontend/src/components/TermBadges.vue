...
import { termLabel, termQualityClass, termRange } from '@/utils/format'
...
<template>
  <div v-if="terms.length" class="flex flex-wrap gap-1">
    <button
      ...
      :title="term.desc.replace('{v}', valueText(term))"
    ...
    <Teleport to="body">
      ...
      >
        ...
        <p class="text-ink-200">{{ active.desc.replace('{v}', valueText(active)) }}</p>
        <p v-if="activeRange" class="text-ink-400">
          数值范围（{{ activeRange[0] }} ~ {{ activeRange[1] }}）
        </p>
        <p class="text-ink-500">触发：{{ active.trigger }}</p>
      </div>
    </Teleport>
  </div>
</template>
