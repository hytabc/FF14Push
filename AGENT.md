...
- **2026-09 三大玩法难度重锚 + 奖励调整**（按线上真实战力 / 伤害分布标定）：① **高难副本**最后两关（`raid_h1` / `raid_h2`）**取消装备词缀与品质限制**（`minAncientTermsPerItem: 0`、`minEquipRarity: common`；**三处必须同改**：`shared/data/raids.json` / `shared/data/balance.json` / `services/balance_defaults.py`，因为 `balance.json` 会覆盖 `raids.json`），改以更高战力门槛（62000 / 62500）与更厚的 BOSS（`hpMultiplier` 2400 / 1000、`attackMultiplier` 7.0 / 5.6；按「满配进场」使预测耗时≈可存活时间，即约 50% 理论通关率）维持难度，金币奖励大幅提高（首通 2300 万 / 3100 万）。② **远征**：`multiplayer.json` 的 `gateScale`（极 0.78 / 零式 0.82 / 绝 0.82）与 `phases[].hpScale`（170 / 215 / 230）按真实玩家能力上界重定（`references` 与 `seed 20260922` **保持不变**——它是引擎压力 / 职责检查公式的标定输入，改动会打破 `test_multiplayer.py::test_all_dungeons_reference_can_clear` 契约）；普通合作与高难奖励全面上调，各副本 `reward.cards` 按难度 1/2/3。
- **远征通关奖励的模式倍率**：`multiplayer.json:modeRewardMultiplier`（`online` ×1.2，`solo` / `offline` ×1.0），在 `services/coop_rewards.grant_reward` 对金币 / 经验 / 卡统一乘算，回执里带 `rewardMultiplier` / `baseGold` 供前端展示。
- **「重新打造卡」**（`consumables.json` 的 `recraft_card`，堆叠 `kind="card"`，注意它**不是**药食）：远征极 / 零式 / 绝通关按难度产出（`coop_rewards.grant_reward`）。`POST /economy/refine`、`/economy/enchant` 支持 `useCard=true`：**仅生产/采集专用装备**、强制 `basedOnCurrent`、每件每次消耗 1 张、**不扣金币**；为此 `services/item_factory.regenerate_attrs` / `roll_terms_for_enchant` 增加专用装备分支（基础属性用 `dohdol_base_attr_range`、词条走 `roll_dedicated_terms`，无副属性），`term_range` 回退查 `dohdol_equipment.terms`（否则专用词条浮动区间恒为 0）。卡可系统出售但不能当药食使用（`consumables.use_consumable` 只接受 potion / food），也**不上交易板**（市场 `StackKind` 白名单不含 `card`）。
- **绝境战通关称号**：`titles.json` 新增 `condition.type = "coop_clear"`（比对 `dungeonIds`），由 `services/titles.evaluate_coop_titles` 按 `CoopProgress.clears > 0` 幂等补发，触发点在 `coop_rewards.grant_reward`；三条称号与 FF14 官方一致——传奇（`ultimate_1`）/ 究极（`ultimate_2`）/ 创世（`ultimate_3`），`matches()` 对新类型返回 False（不影响钓鱼确定性引擎）。
...
- **重建伊修加德** 已实现：**全服共享进度**的生活玩法。配置 `shared/data/ishgard.json`（`stages[]` 按**阶段**划分：第 X 次重建，X=1..5），服务 `services/ishgard.py`，接口 `api/v1/ishgard.py`（前缀 `/ishgard`），页面 `IshgardView.vue`（页内二级 Tab：概览 / 采集 / 钓鱼 / 生产 / 提交 / 天钢装备 / 积分榜）。要点：**物资按阶段划分且不可跨阶段使用**——物资显示名为「第 X 次重建用的…」（`services/dohdol_util.ishgard_display_name`，按物资自身 `stage` 生成），专属采集点 / 钓场 / 配方**只产出当前阶段**的物资，采集 / 生产 / 提交三处服务端都校验「阶段一致」，过期物资只能 `POST /ishgard/sell` 出售（页面背包只显示 `StackItem.kind="ishgard"` 的专属物资，支持按阶段筛选）。**全服阶段进度** `ishgard_state.stage_points` 累加提交积分（`POST /ishgard/submit`，行锁），阈值 `stageTargets`（第 1 阶段 100W，逐级递增），超出部分**结转**，第 5 阶段完成 `round+1` 并回到阶段 1（个人累计积分 / 装备 / 称号**保留**）。**独立积分榜** `ishgard_members.points`（即个人**累计总积分**，进程内短 TTL 缓存 + 写入失效，**不并入总排行榜**）。**周期称号**：`titles.json` 的 `ishgard_saint`（天穹圣人）/ `ishgard_apostle`（天穹圣徒），`condition.type="ishgard_rank"`（不参与确定性判定）；每 6H 由 `ishgard.settle_titles` 结算——`GET /ishgard/state` 与 `POST /ishgard/submit` 惰性触发 + `ranking_worker` 兜底（advisory lock 防重），**全服唯一、严格超越才转移、平局保留现任**，且**同一人不得同时持有两个称号**；持有者由全局单行决定，`UserTitle` 行只对现任存在，被夺者若正佩戴则清空 `users.active_title_id`。**可成长主手**（生产 / 采集各一件）：`ishgard_tools`（`level` 0 或 20/40/60/80/100，固定神话 + 高品质，各段 bonus = `dohdol-equipment` 同段基础型 ×1.2，**仅本玩法产出** `source="ishgard"`，不进抽取 / 打造 / 合成池），**达标领取 + 手动升级**（门槛 = 个人累计积分 ≥ `threshold` 且已完成 `stage` 次重建，不额外消耗材料）。**紫色附魔**为独立于词条的固定 1 条（`ishgard_tools.pink_values` JSON，8 条 = 生产 4 / 采集 4），仅 `recraft_card` 可重铸（`POST /ishgard/tool/enchant`，不扣金币、不动基础属性与词条）。紫色附魔与装备基础加成同源：`dohdol_util.equipped_bonus` 已把 `ishgard_tool_by_id` 视作专用装备底材，采集 / 钓鱼 / 生产**三个既有页面**与重建页面都改用 `ishgard.bonus_with_purple` 合并（未装备不计）。表：`ishgard_state` / `ishgard_members` / `ishgard_tools` / `ishgard_contributions`（迁移 `a4c6e8b0d2f4`，**只建表不清理**；贡献行永久保留）。边界：物资存专属 stack kind，不进普通材料 / 鱼池与交易板；`shared/schema/{loader.py,index.ts}` 需同步维护 `ishgard` 索引。
- 界面及项目文档主要使用中文，新增内容保持已有命名和文案风格。
- **死者宫殿**（roguelike 深层迷宫）已实现：**与账号战力完全隔离**的玩法——副本内英雄由 `services/recruiting.generate_candidate(..., allow_egg=False)` 随机生成 3 选 1、武器由 `services/item_factory.generate_item(category="weapon", ...)` 随机生成 3 选 1，副本内英雄 1 级起步、装备 / 金币 / BUFF / 等级全部只存 run 快照（**不落 `items` 表、不读账号面板 / 秘药 / 魔晶石 / 周目难度**）。配置 `shared/data/palace.json`（层 / 步、节点权重、怪物曲线、战斗奖励权重、层 BOSS 奖励、副本 BUFF 池、商店、兑换表）由 `scripts/gen-palace-data.py` 生成，另有 `palace-growth.json`（局外成长树 24 类别 × 10 级 = 240 节点、全树 528 点）与 `palace-events.json`（36 条事件）。服务 `services/palace.py`（run 生命周期 + 成长 / 兑换）、`palace_map.py`（每层 DAG：1对1 / 1对多 / 多对1，第 10 步固定层主）、`palace_data.py`（英雄 / 装备 / 敌人 / 奖励生成）、`palace_stats.py`（run 面板 = `stats.compute_stats` 复用 transient hero/items + `dataclasses.replace` 叠加成长与 BUFF，**前后端同一 `HeroStats`**）、`palace_balance.py`（**逐节点服务端校验**：进入冻结快照、结算复算时长下限 + 时钟容差，照 `raid_balance` 范式）、`palace_sim.py`（无头标定）。接口 `api/v1/palace.py`（前缀 `/palace`），页面 `PalaceView.vue`（页内 Tab：概览 / 路径 / 装备 / 成长 / 兑换，独立战斗与装备 UI，含 `components/PalacePathMap.vue`），store `stores/palace.ts`，路由 `/palace` + 导航「战斗」分组。要点：**零局外成长即使运气极好也只能到第 5 层、满成长可秒杀前 3 层**（`scripts/palace-balance.py` 实测 + `tests/test_palace.py` 数值断言）；击败第 F 层层主给成长点与代币（1-5 层烈火纹章 / 6-10 层玻璃南瓜）；通关第 10 层 10 次解锁称号「死灵术士」（`titles.py:evaluate_palace_titles`，`condition.type="palace_clear"`）。表：`palace_profiles`（账号级成长点 / 代币 / 通关次数 / 已解锁节点）、`palace_runs`（每账号一份 run 快照，迁移 `b5d7f9a1c3e5`，**只建表不清理**）。活动互斥：`services/roster.end_palace_runs` 已接入 `stop_activities`（开始其它活动即结束本次 run，已入账奖励保留）；退出页面 / 断线 run 保留，重进可续。设计见 `docs/palace-of-the-dead-design.md`。
...
- **Shell 脚本：`$变量` 后面紧跟中文/全角字符时必须写成 `${变量}`**（如 `log "版本：${VERSION}（build ${BUILD_NUMBER}）"`）。macOS 自带 **bash 3.2**，在 UTF-8 locale（用户交互式 shell 的默认环境）下会把紧随其后的多字节字符的首字节并入变量名，报成 `xxx?: unbound variable`；而 `LC_CTYPE=C` 的环境（例如某些工具的执行环境）**不会复现**，所以本地跑通不代表用户那边跑得通。新增脚本用 `python3 -c` 或类似方式扫一遍 `\$[A-Za-z_]\w*(?=[^\x00-\x7F])` 再提交。
...
- **生产 / 采集 / 钓鱼地区解锁按「账号已通关的最远地区」**：`build_game_state` 的 `regionProgress[*].unlocked` 用 `services/gathering.cleared_max_region(db, user_id) + 1` 判定（与 `start_gather` / `start_fish` 的服务端门槛同源），**与当前上场英雄的面板无关**——切到低等级英雄不会把已解锁的采集地区重新锁上。`region_access`（按英雄面板的 `region_gate`）只保留给 `currentRegion` 判定与战斗列表（`/region`），**不要**再拿它算 `regionProgress`。
