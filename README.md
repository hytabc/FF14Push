...
高难进入要求等级、装备质量、战力、主攻击和双防达标；正式通关还要求输出检查、防御检查及服务端时长校验。每次高难尝试保存版本、资格和倍率快照；过期无快照会话需重新挑战。
...
部署前执行数据库迁移：`cd backend && .venv/bin/alembic upgrade head`。迁移 `g4b6c8d0e2f4` 添加挑战快照与结果列，`c3d4e5f6a8b9` 移除机制试炼表，均不重写历史数据。已有角色资格按请求即时重算，无需批量重写装备或奖励。
