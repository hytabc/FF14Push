...
| 聊天室 | 单一公开大厅，登录后可看可发，实名展示 `昵称#登录账号`（与排行榜一致，管理员带「管理员」徽章）。**仅文本**（单条 ≤200 字），**不保留聊天记录**：服务端只保留最近 10 分钟的消息（`services/chat.RETENTION_SECONDS`），窗口外的历史永不返回；**发言限频 20 条 / 60 秒**（`guard_rate`）防刷屏。实时推送用 WebSocket（`/chat/ws`，短时效一次性 ticket 鉴权），参照远征的 DB 轮询广播，多 worker 安全。管理员可在聊天室**发布公告**（高亮展示，不受发言限频） |
