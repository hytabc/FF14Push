...
| 系统 | 说明 |
| --- | --- |
| 采集 | 「采矿工 / 园艺工」按地区采集：**不同地区采集不同材料**——每个地区 2 件专属材料（矿物 + 植物，共 80 件、**跨地区不重复**，FF14 风格命名），另可采到少量通用材料，故每地区可采 ≥3 种；材料不随地区等级递增、仅按种类区分。后台自动循环，产出由服务端按采集等级 + 专用装备 + 药水加成结算 |
| 生产 | 8 个能工巧匠按配方制造材料 / 半成品 / 战斗装备 / 专用装备 / 药水食物。配方按**生产等级**解锁。经验 = 配方基础 `xp` ×「品阶经验系数」×（1 + 制造经验加成）；其中装备按实际抽到的品阶乘 `recipes.json:equipment.xpRarityMultiplier`（普通 1.0 → 神话 3.0，**品阶越高经验越多**），材料 / 半成品 / 消耗品无品阶，恒按 1.0 |
...
| 药水 / 食物 | 炼金术士制药水（60s，效果强）、烹调师制食物（1800s，效果弱），**可共存**、手动使用。**同一件连续使用时长叠加**（在剩余时间之上再累加一份时长）；换成同槽位的另一种则重置为新的一份时长。效果覆盖经验/金币、抽箱品阶概率、制造品质概率、捕鱼人之识时长、采集产量；**不影响战力与门槛** |
...
生产配方、本次生产和药食库存直接展示秘药/料理的全部加成及持续时间。普通战斗、地区 BOSS 和副本通关的经验日志采用服务端结算值，括号显示相对结算基础的实际增幅、效率加成和追赶加成；结算基础已包含怪物倍率与奖励效率修正。技能直接治疗及持续治疗记录实际恢复生命与溢出治疗量。
