...
- **变体**：同档位多套底材（见 `base-items.json:variants`），名称中段为 FF14 官方职能装备词缀（御敌/强袭/制敌/游击/精准/治愈/咏咒），装备名 = 档位 + 职能 + 部位（如「精金强袭铠甲」），与 FF14 命名惯例一致。除副属性池与档位缩放不同外，图标按变体做**色调混合**（暴击=红、直击=青、信念=紫、力量/敏捷/智力…）并在右上角加变体角标，因此同档不同变体一眼可辨。
