...
| 药水 / 食物 | 炼金术士制药水（60s，效果强）、烹调师制食物（1800s，效果弱），**可共存**、手动使用。**同一件连续使用时长叠加**（在剩余时间之上再累加一份时长）；换成同槽位的另一种则重置为新的一份时长。效果覆盖经验/金币（经验加成对战斗 / 生产 / 采集 / 钓鱼均生效）、抽箱品阶概率、制造品质概率、捕鱼人之识时长、采集产量；**不影响战力与门槛** |
