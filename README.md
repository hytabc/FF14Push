...
| 附魔 | 重掷全部词条，稀有 1.0% / 太古 0.1%，支持自动追梦；太古词条以 🌟 标记 |
| 高难副本 | 4 个副本（2 单 BOSS + 2 双 BOSS）：无小怪、属性按英雄等级锚定后大幅放大、BOSS 自带抗性；双 BOSS 可切换目标，一方阵亡后另一方狂暴（逼同步击杀）。门槛 = 等级 + 战力 + 11 栏穿满；首通重奖、重刷少量金币，通关时间由服务端按理论下限校验 |
...
| 高难副本 | PRD 无此系统，为新增内容。副本无小怪、只有 BOSS；属性以**英雄等级**锚定（与地区一致），倍率与抗性见 `shared/data/raids.json`；门槛为等级 + 战力 + 栏位穿满。奖励首通重奖、重刷仅少量金币，避免成为金币放大器 |
