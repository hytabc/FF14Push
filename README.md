...
| 排行榜 | 等级榜 / 关卡榜（必做）+ 战力榜 / 金币榜，每 5 分钟服务端刷新；昵称后以 0.6 透明度显示 `#登录账号`（如 `ABCD#zhangsan`）便于区分重名（昵称可重复，账号唯一）。**点击任意玩家可查看其当前装备**（只读，仅已装备栏位；需登录，封禁/无英雄账号返回 404）。弹窗内含「战斗装备 / 生产采集装备」页签，可查看对方的生产 / 采集专用装备 |
...
| 制造装备 | **恒为「高品质」**：属性区间整体上移（`recipes.equipment.highQualityMultiplier`）且**必带 1 条太古词条**；同时按 `rarityWeights` 独立抽白/绿/蓝/紫/橙/红品阶。同品阶下强于抽奖装备 |
| 专用装备 | 生产 / 采集各有独立栏位（主手工具 + 副手工具 + 5 防具），**仅能通过生产获取**，只影响采集产量 / 制造品质 / 钓鱼概率，**不参与战斗结算、战力榜、地区与副本门槛** |
...
| 药水 / 食物 | 炼金术士制药水（60s，效果强）、烹调师制食物（1800s，效果弱），**可共存**、手动使用。效果覆盖经验/金币、抽箱品阶概率、制造品质概率、捕鱼人之识时长、采集产量；**不影响战力与门槛** |
| 排行榜 | 新增**两个独立榜单**：**钓鱼种类榜**、**钓鱼数量榜**（只有有钓鱼记录的玩家上榜）。这两个榜**实时**从 `fish_records` 聚合（刚钓完就能看到，不等 5 分钟缓存）；种类榜还会按**普通 / 鱼王 / 鱼皇**拆开显示种类数。四个基础榜仍走 5 分钟缓存。行内展示称号徽章 |
...
药水 / 食物到期同样以服务端时钟判定。新表见迁移 `f2a4b6c8d0e2`。
