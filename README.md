...
| 副本战力缩放 | 副本 BOSS 原本只按英雄**等级**锚定，而战力主要来自**装备品阶**（与等级无关），导致门槛一满足就能碾压（实测 Lv49 满装打 raid_1 仅 0.4s）。现改为 `BOSS血量 ×(玩家DPS ÷ 本等级参考DPS)^0.8`、攻击 ×同比例 `^0.4`；参考 DPS = 等级锚定 DPS × 各难度系数，即「刚好够门槛的装备」的输出。比值下限为 1，所以只有超模装备才会触发（达标装实测单场约 25-45s）。见 `shared/data/raids.json:balance` |
