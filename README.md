...
| 排行榜 | 等级榜 / 关卡榜（必做）+ 战力榜 / 金币榜，每 5 分钟服务端刷新；昵称后以 0.6 透明度显示 `#登录账号`（如 `ABCD#zhangsan`）便于区分重名（昵称可重复，账号唯一）。**点击任意玩家可查看其当前装备**（只读，仅已装备栏位；需登录，封禁/无英雄账号返回 404） |
...
- 生产·采集 DLC 的材料 / 配方 / 鱼类 / 消耗品内容为首版（每地区 2 件专属材料共 80 件、4 种普通鱼 + 鱼王 + 鱼皇），
  由 `scripts/gen-dohdol-data.py` 生成、结构与数值集中可调；鱼类 / 材料图鉴未实现。
