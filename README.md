...
| 排行榜 | 新增**两个独立榜单**：**钓鱼种类榜**（鱼类种类数降序）、**钓鱼数量榜**（累计钓鱼条数降序）；只有有钓鱼记录的玩家上榜。行内展示称号徽章（`RankingEntry.payload`，免迁移） |
...
采集 / 生产 / 钓鱼的开始与上报响应都会带上 `cycle`（单次动作耗时 + 未结算余额），前端据此
**插值画进度条**——进度条只是展示，结算仍以服务端为准。
