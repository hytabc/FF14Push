...
| 出售 | 底价 × 品阶系数 × 属性系数（饱和封顶）× 词条价值 × ±10%，支持批量与自动出售；**抽箱中被自动出售的装备仍参与抽奖动画，出售结算在结果页展示**（未入背包）；**所有出售入口（含批量）都有二次确认** |
| 穿戴 | 英雄职能由**已装备的主手武器**决定；防具 / 饰品带职能词缀（御敌 / 强袭 / 制敌 / 游击 / 精准 / 咏咒 / 治愈），只能穿脱与当前武器职能一致的装备；**换武器会自动卸下**不符职能的装备。基础型（无词缀）与世界BOSS 专属防具不受限 |
...
| 排行榜 | 等级榜 / 关卡榜（必做）+ 战力榜 / 金币榜 / **游玩时间榜**，每 5 分钟服务端刷新；昵称后以 0.6 透明度显示 `#登录账号`（如 `ABCD#zhangsan`）便于区分重名（昵称可重复，账号唯一）。**战力榜按「历史最高战力」排行**（`users.max_power`，迁移 `d5e7f9a1b3c5`）：在每次算出当前战力的热路径上只增不减地抬高（`/game/state` + 排行榜刷新），行内同时显示当前战力，换下装备 / 降级后名次不回退。**点击任意玩家可查看其当前装备**（只读，仅已装备栏位；需登录，封禁/无英雄账号返回 404）。弹窗内含「战斗装备 / 生产采集装备」页签，可查看对方的生产 / 采集专用装备，并显示其累计游玩时间与历史最高战力 |
...
| 药水 / 食物 | 炼金术士制药水（60s，效果强）、烹调师制食物（1800s，效果弱），**可共存**、手动使用。分 **I / II / III 三档**：档位越高效果越强（×1 / ×2 / ×4，如经验获取 +25/+50/+100%），但**单个物品的持续时长不变**；II / III 档配方分别需生产等级 40 / 80，并用对应地区的采集材料（`flora17`/`ore17`、`flora32`/`ore32`）。**同一件连续使用时长叠加**（在剩余时间之上再累加一份时长）；换成同槽位的另一种则重置为新的一份时长。效果覆盖经验/金币（经验加成对战斗 / 生产 / 采集 / 钓鱼均生效）、**抽箱品阶概率**、**制造品阶概率**、制造品质概率、捕鱼人之识时长、采集产量；**不影响战力与门槛**。**食物 / 秘药悬浮窗常驻全站**（可拖动 / 最小化），每项带「续期」按钮：消耗背包中同种物品即可叠加一份时长（未持有则按钮不可用） |
...
| 物品出售 | **采集材料 / 半成品 / 鱼获 / 药水食物均可卖给系统换金币**（单价见各 JSON 的 `sell` 字段，按地区档位/稀有度递增）。见 `POST /dohdol/sell`；采集页、生产页、钓鱼页、背包都有「出售 / 全部出售」按钮，**均带二次确认** |
