...
| 副本 | 6 个副本，分两档：**普通**（4 个，Lv20-80）门槛 = 穿满 + 全部 ≥蓝色 + 至少 6 件紫色；**高难**（2 个，Lv80/100）门槛 = 穿满 + 全部 ≥传说 + 至少 6 件神话 + **每件装备至少 1 个太古词条**。无小怪、属性按英雄等级锚定后大幅放大、BOSS 自带抗性；**BOSS 还会按玩家战力相对本等级的参考值缩放**（超模装备不再碾压）。双 BOSS 可切换目标，一方阵亡后另一方狂暴（逼同步击杀）。首通重奖、重刷少量金币，通关时间由服务端按理论下限校验 |
| 副本压力 | BOSS 攻击力经**真实模拟器标定**：门槛装备通关后只剩 14%-57% 生命，弱一档就阵亡——无法靠「拖时间磨过去」（改前是全程 100% 生命，等于免伤） |
| 英雄招募 | 酒馆刷新与金币招募，资质 6 阶，三维按偏向分配 |
| 图鉴 | 装备（按底材 × 品阶）、怪物、词条（普通/稀有/太古三档） |
| 排行榜 | 等级榜 / 关卡榜（必做）+ 战力榜 / 金币榜，每 5 分钟服务端刷新；昵称后以 0.6 透明度显示 `#用户ID`（如 `ABCD#1234`）便于区分重名 |
...
| 高难副本 | PRD 无此系统，为新增内容。副本无小怪、只有 BOSS；属性以**英雄等级**锚定（与地区一致），倍率与抗性见 `shared/data/raids.json`。门槛分两档：普通副本要求全部 ≥蓝色且至少 6 件紫色，高难副本要求全部 ≥传说、至少 6 件神话且**每件至少 1 个太古词条**。奖励首通重奖、重刷仅少量金币，避免成为金币放大器 |
...
| 副本攻击标定 | 副本 BOSS 攻击力不靠手算：用真实客户端模拟器（`BattleSimulator`）逐档扫描，「门槛装备通关后剩余生命」作为难度指标定档。见 `shared/data/raids.json` 各 boss 的 `attackMultiplier`；回归保护见 `tests/test_engine.py:TestRaidPressure` |
| 副本战力缩放 | 副本 BOSS 原本只按英雄**等级**锚定，而战力主要来自**装备品阶**（与等级无关），导致门槛一满足就能碾压（实测 Lv49 满装打 raid_1 仅 0.4s）。现改为 `BOSS血量 ×(玩家DPS ÷ 本等级参考DPS)^0.8`、攻击 ×同比例 `^0.4`；参考 DPS = 等级锚定 DPS × 各难度系数，即「刚好够门槛的装备」的输出。比值下限为 1，所以只有超模装备才会触发（那之后 raid_1 满装约 28s、raid_4 约 99s）。见 `shared/data/raids.json:balance` |
