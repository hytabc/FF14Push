...
| 系统 | 说明 |
| --- | --- |
| 采集 | 「采矿工 / 园艺工」按地区采集：**不同地区采集不同材料**（每地区一件特产），材料不随地区等级递增、仅按种类区分。后台自动循环，产出由服务端按采集等级 + 专用装备 + 药水加成结算 |
...
| 排行榜 | 新增两列：**钓鱼种类数**、**钓鱼数量**；行内展示称号徽章（`RankingEntry.payload`，免迁移） |
...
- 生产·采集 DLC 的材料 / 配方 / 鱼类 / 消耗品内容为首版（40 地区各 1 件特产材料、4 种普通鱼 + 鱼王 + 鱼皇），
  结构与数值集中可调，后续可按同一 schema 扩写；鱼类 / 材料图鉴未实现。
