...
| 触发效果 | 词条支持概率触发（proc）：**灼烧**（命中概率给目标挂 DOT）、**疾风**（命中概率叠限时攻速）。概率由词条值承载，效果量在 `combat.json:proc`；前端 `battle.ts` 结算，后端 `combat_model.proc_dps_bonus` 计入期望。**攻速**（sks 副属性 / 迅捷 / 疾风）会缩短 GCD 与普攻间隔 |
...
| 药水 / 食物 | 炼金术士制药水（60s，效果强）、烹调师制食物（1800s，效果弱），**可共存**、手动使用。**同一件连续使用时长叠加**（在剩余时间之上再累加一份时长）；换成同槽位的另一种则重置为新的一份时长。效果覆盖经验/金币（经验加成对战斗 / 生产 / 采集 / 钓鱼均生效）、**抽箱品阶概率**、**制造品阶概率**、制造品质概率、捕鱼人之识时长、采集产量；**不影响战力与门槛** |
...
共 **1908 张**：战斗装备 935（30 个部位底形 × 9 个档位 × 变体）+
采集材料 / 半成品 98 + 生产·采集专用装备 623 + 鱼获 240 + 药水食物 12。
...
- 单张约 130 字节、合计约 259 KB，低于 Vite 默认 `assetsInlineLimit`，构建时全部内联为 data URI ⇒ **渲染物品时零图片请求**。
