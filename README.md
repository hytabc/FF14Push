...
高难进入要求等级、装备质量、战力、主攻击和双防达标；正式通关还要求本副本机制试炼、输出检查、防御检查及服务端时长校验。缺试炼标记可练习，但不发正式通关奖励。试炼由服务端随机下发顺劈/冲击/蓄力提示，连续三次在时限内作出侧移/防御/打断响应；不可由挂机或低难刷取替代。每次高难尝试保存版本、资格和倍率快照；过期无快照会话需重新挑战。

新地区必须同时满足战力、装备质量、前一地区 BOSS 通关、主攻击、双防及该地区试炼。初始地区 1 是默认开放的新手入口。默认 `migration = "recalculate"`：列表、进入、推进、当前地区和战斗上报均重算资格，旧解锁标记不能绕过新条件；角色、装备、奖励、历史通关与最快记录均保留。`new_unlocks_only` 可保留已解锁地区，后续新地区仍必须通过全部条件。
...
45%–55% 是上线后的观测目标，合成样本测试不能证明真实玩家首通率已达标。运维应依据滚动窗口，仅修改版本化门槛/战斗参数，不改历史通关记录；发布不兼容资格规则时提升 `version`，旧试炼标记随之失效。

部署前执行数据库迁移：`cd backend && .venv/bin/alembic upgrade head`。新增迁移 `g4b6c8d0e2f4` 仅添加挑战快照、结果列和机制试炼表，不重写历史数据。已有角色资格按请求即时重算，无需批量重写装备或奖励。
...
| 副本 | 普通开放低战力挑战；高难要求等级、装备、战力、关键属性和机制能力。实际门槛及遥测规则见 `balance.json` 和「战力平衡 v2」 |
